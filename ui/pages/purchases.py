...
def _statement_table(items: pd.DataFrame, purchase_module) -> pd.DataFrame:
    rows = items.copy()
    for col in ["정식제품명", "규격", "입고수량", "포장단위", "단위"]:
        if col not in rows.columns:
            rows[col] = ""
...
""",
    ...
    for order_id in order_ids:
        ...
        with st.expander(label, expanded=False):
            ...
            m2.metric("합계 입고수량", f"{int(total_received):,}")
            m3.metric("합계 총 매입금액", f"{total_purchase:,}원")
            ...
            for seq, (_, statement) in enumerate(linked_statements.iterrows(), 1):
                ...
                t1, t2 = st.columns(2)
                t1.markdown(
                    f'<div class="statement-total-box">총 입고수량&nbsp;&nbsp;{received_qty:,}</div>',
                    unsafe_allow_html=True,
                )
                t2.markdown(
                    f'<div class="statement-total-box">총 매입금액&nbsp;&nbsp;{statement_total:,}원</div>',
                    unsafe_allow_html=True,
                )
